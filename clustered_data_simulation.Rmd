---
title: "Clustered Data Simulation Function"
output:
...
This document illustrates how to generate clustered data and simulate potential outcomes using the `dplyr` package.
```{r}
...
simulate_cluster_outcome <- function(dat.cluster,
  ...
  # if L_i is needed but not already in data, generate it, p_L is used for generate L_i
  if ("L_i" %in% c(outcome_covars, treatment_covars) && !("L_i" %in% names(dat.cluster))) {
...
# True Casual Effect Generation
```{r}


simulate_cluster_true_effect <- function(PO,
                                         dat.cluster,
...
# Testing for adding covariates in the outcome and treatment model
```{r}
...
dat.cluster$Li <- runif(nrow(dat.cluster))


# Li is default in the equation,which is needed, x1,x2,x3 are the added_covariates.
...
# If want to to see the outcome and treatment model 
out <- simulate_cluster_outcome(
  dat.cluster,
...
# Testing for the true casusal effect
```{r}

sim_out <- simulate_cluster_outcome(dat.cluster, define_observed_outcome = FALSE)
